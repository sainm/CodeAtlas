...
- [ ] Method-local alias chains are tracked for request parameters and method parameters, for example `request.getParameter("x") -> a -> b -> service.save(b)`.
- [ ] Method-local expression transformations preserve variable sources, for example `request.getParameter("x") -> raw.trim() -> String.valueOf(raw) -> service.save(...)`.
- [ ] Direct expression arguments preserve variable sources, for example `service.save(raw.trim())`.
- [ ] Method-local alias propagation respects source order and does not retroactively taint calls before the assignment.
- [ ] Variable trace evidence includes the Java source file path and line number for request parameter reads/writes and method argument flows.
- [ ] Struts `ActionForm form` aliases such as `(UserForm) form` can propagate getter reads like `getUserId()` into downstream `PASSES_PARAM` facts.
- [ ] Struts `DynaActionForm` aliases can propagate `get("property")` and `getString("property")` reads into downstream `PASSES_PARAM` facts.
- [ ] Request-derived argument flows also create `READS_PARAM` facts on the caller method, allowing variable sink queries to start from the request parameter.
- [ ] Business jars under `WEB-INF/lib` can be indexed as class and method nodes.
- [ ] Business jar classfile type flags create proper CLASS, INTERFACE, ENUM, and ANNOTATION nodes.
- [ ] Business jar class fields create FIELD nodes and class `DECLARES` facts.
...
- [ ] Method nodes expose code-origin metadata: `codeOrigin`, `hasSource`, `hasJvm`, `sourceOnly`, `jvmOnly`, `synthetic`, and `bridge`.
- [ ] Generic bridge methods generated by javac are marked as both `synthetic=true` and `bridge=true` when read from class files.
- [ ] Business jar class nodes preserve class-level annotation names from classfile annotation attributes.
- [ ] Business jar entity classes can produce default class-to-table and field-to-column `BINDS_TO` facts from classfile annotations.
- [ ] Business jar entity mapping reads classfile annotation string values such as `@Table(name=...)`, `@Column(name=...)`, and `@JoinColumn(name=...)`.
- [ ] Business jar static fields and transient fields are excluded from default entity field-column facts.
- [ ] Business jar Spring `Controller`/`RestController` classes can produce API endpoint `ROUTES_TO` facts from method-level mapping annotations.
- [ ] Business jar field nodes expose `static` metadata so downstream analyzers can skip non-instance state.
...
- [ ] Spring mapping paths from `value` and `path` annotation attributes are read through AST annotation values, not source text regex.
...
- [ ] Spring qualifier names are read through AST annotation values, not source text regex.
...
- [ ] `parameter="method"` can route to matching DispatchAction methods packaged inside business jars.
- [ ] `LookupDispatchAction#getKeyMethodMap()` simple string-literal `Map.put` entries create keyed `LIKELY` method route facts.
- [ ] `LookupDispatchAction#getKeyMethodMap()` mapping extraction uses Java AST facts, not source text regex.
...
- [ ] Struts action mappings can route to `Action#execute` methods packaged inside business jars under `WEB-INF/lib`.
- [ ] ActionForm subclasses packaged inside business jars bind non-static fields to request parameters, while static fields are skipped.
- [ ] Action `mapping.findForward("name")` calls create method-to-forward config facts that can connect to configured JSP/action/tiles forward targets.
- [ ] Direct `new ActionForward("/path.jsp")` and `new ActionForward("/path.do")` constructors create method-to-JSP/action forward facts.
- [ ] Named `new ActionForward("name", "/path.jsp", redirect)` constructors use the path argument, not the forward name, for navigation facts.
- [ ] `response.sendRedirect("/path.do")` and `new ActionRedirect("/path.jsp")` create method-to-action/JSP navigation facts.
- [ ] Custom ActionMapping classes are modeled from `<action-mappings type="...">` and per-action `className`.
...
- [ ] MyBatis annotation SQL supports string and string-array annotation values through AST annotation values, not source text regex.
...
- [ ] PreparedStatement setter calls such as `ps.setString(1, name)` create `PASSES_PARAM` facts from the Java method to the JDBC SQL statement.
- [ ] JPA/Hibernate entity classes annotated with `@Entity` create `BINDS_TO` facts to database tables from `@Table(name=...)` or the entity class name.
- [ ] JPA entity fields create `BINDS_TO` facts to database columns from `@Column(name=...)`, `@JoinColumn(name=...)`, or the field name.
- [ ] Static fields and fields annotated with `@Transient` are excluded from JPA field-column facts.
...
- [ ] Agent and MCP guards reject write-capable tool calls unless `confirmWrite=true` and `confirmationIntent=ALLOW_WRITE` are present.
