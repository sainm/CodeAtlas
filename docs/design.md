...
7. Commit
   原子写入在 FactStore 同一事务内完成校验、upsert 和 tombstone，发布 committed snapshot；失败回滚旧事实无损。
...
### 9.3 FactStore 抽象与容量感知

FactStore（`FactStore` 接口）抽象事实存储后端，支持内存（`InMemoryFactStore`）和 Neo4j 两种实现：

- `InMemoryFactStore` 是开发、CI 和单机场景的默认后端，使用 `ReadWriteLock` 实现 single-writer coordination。
- Neo4j 是多用户、大规模生产部署的推荐后端。`StoreSizing` 在单项目 active facts 超过 50 万时自动告警推荐切换。
...
- 默认使用 `InMemoryFactStore`，可通过 `ImpactAnalysisService.using(neo4jStore)` 切换到 Neo4j。
...
| **存储** | InMemoryFactStore（dev/CI/小项目默认） | 50万 facts 告警推荐切换 | — | 单机要求 Neo4j |
| **API/MCP** | 14 只读 MCP tools、REST /api/v1、分页/错误/审计 | 部分 endpoint 返回 stub | — | 写操作 MCP tools |
...
| Neo4j / traversal query | 5s | BFS depth ≤ 5, max paths ≤ 100 |
...
- Git diff → changed method → caller → entrypoint
- DB table/column → SQL statement → Mapper/DAO → Service → entrypoint（区分 read/write/display）
- 变量/参数 → getter/setter 链 → SQL 参数
- Git diff → FastImpactReport（含 changed/affected symbols、paths、DB impacts、truncation）
- 能用 AI 生成简洁风险摘要和测试建议。
...
以下能力在 MVP 阶段明确标记为 candidate-only 或 unsupported，不作为生产门禁阻塞项：
...
Joern/Tabby/WALA/SootUp/Tai-e/Doop **不进 MVP 关键路径**，也不进默认发行包：

- Joern、Tabby：作为 depth CPG/slicing 候选侧车，独立 worker 运行，结果进入 candidate family。
...
- `define(name, description, truePositives, falsePositiveCandidates)` 定义标注数据集。
- `evaluate(reportedPaths)` → `EvaluationResult(totalReported, truePositive, falsePositive, falseNegative, precision, recall)`。
